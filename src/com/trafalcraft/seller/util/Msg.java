...
public enum Msg {
    ...
    public static void load() {
        ...
        BUY_UNAVAILABLE_INVENTORY.replaceBy(Main.getPlugin().getConfig().getString("Msg.sellerInventory.buy_unavailable_inventory").replace("&", "§"));
        SELL_UNAVAILABLE_INVENTORY.replaceBy(Main.getPlugin().getConfig().getString("Msg.sellerInventory.sell_unavailable_inventory").replace("&", "§"));
        //transaction
        ...
        for (String text : Main.getPlugin().getConfig().getStringList("Msg.transaction.failure_buy_no_enough_money")) {
            text1.append(text).append("\n");
        }
